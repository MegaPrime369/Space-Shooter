...
class PowerUp(MenuShip):
    def __init__(
        self, price, img_path, size, background_path, screen, offset, spec, number
    ):
    ...
    def create_divided_texts(self):
        ...
        if self.spec_image_rect.width > self.background_rect.width:
            ...
            self.text_1_image = self.spec_font.render(
                self.text_1, True, self.spec_color
            )
            ...
            self.text_2_image = self.spec_font.render(
                self.text_2, True, self.spec_color
            )
    ...
    def create_status(self):
        ...
        self.platform = pygame.transform.smoothscale(
            self.platform, PLATFORM_SIZE
        ).convert_alpha()
        ...
        self.number_image = self.text_font.render(
            str(self.number), True, self.button_text_color
        )
